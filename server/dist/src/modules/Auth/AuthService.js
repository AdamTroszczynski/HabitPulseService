...
import { changeUserPassword, createUser, getUserByEmail, getUserById, verifyUser, } from '../../repositories/User/UserRepository.js';
...
export const logoutService = async (dto) => {
...
export const registerService = async (dto) => {
    const user = await getUserByEmail({ email: dto.email });
    if (user)
        throw new AppError(ErrorCodes.CONFLICT, 'User already exists', HttpStatus.CONFLICT);
    ...
    getChannel().publish('habitpulse.messages', 'email', Buffer.from(JSON.stringify({ type: 'activation', email: newUser.email, token })), { persistent: true });
};
export const activateService = async (dto) => {
    const authToken = verifyAuthToken(dto.token);
    if (authToken.type !== 'activation')
        throw new AppError(ErrorCodes.UNAUTHORIZED, 'Invalid credentials', HttpStatus.UNAUTHORIZED);
    const user = await getUserById({ id: authToken.userId });
    if (!user)
        throw new AppError(ErrorCodes.NOT_FOUND, 'User does not exist', HttpStatus.NOT_FOUND);
    if (user.isVerified)
        throw new AppError(ErrorCodes.CONFLICT, 'User is already verified', HttpStatus.CONFLICT);
    await verifyUser({ id: authToken.userId });
};
export const resendActivationService = async (dto) => {
    const user = await getUserByEmail({ email: dto.email });
    if (!user)
        throw new AppError(ErrorCodes.NOT_FOUND, 'User does not exist', HttpStatus.NOT_FOUND);
    if (user.isVerified)
    ...
    getChannel().publish('habitpulse.messages', 'email', Buffer.from(JSON.stringify({ type: 'activation', email: user.email, token })), { persistent: true });
};
export const resetPasswordService = async (dto) => {
    ...
    getChannel().publish('habitpulse.messages', 'email', Buffer.from(JSON.stringify({ type: 'resetPassword', email: user.email, token })), { persistent: true });
};
export const changePasswordService = async (dto) => {
    const authToken = verifyAuthToken(dto.token);
